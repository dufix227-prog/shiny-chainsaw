...
- **Главное меню** (`scenes/menu/`): «Новая игра» → имя котика → стартовая катсцена → проба графики (пока участок 40 м не переделан в новом стиле). «Продолжить» — последний снимок, «Загрузить» — 20 слотов, «Настройки».
...
- Проверки: `tests/menus_and_saves.gd`, `tests/intro.gd`, `tests/style_probe.gd`, `tests/first40.gd`.
