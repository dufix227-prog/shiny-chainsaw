...
## Как сейчас устроена игра (25.09.2026)

- **Главное меню** (`scenes/menu/`): «Новая игра» → имя котика → стартовая катсцена → **стартовое место** (`scenes/world/start_area.tscn`): управление начинается там, где кончилась катсцена, первые 10 м тропы до временного завала. Можно вернуться к дороге — машина запускает концовку С9-машина («глупый суицидник»: полёт → отдаление → «Конец» → титры-заглушка → выход). Мир — общая сцена `start_area_world.tscn` для катсцены и игры. «Продолжить» — одна кнопка: последний снимок или выбор другого из 20 слотов; «Настройки».
- **Катсцена С1** (`scenes/cutscene/intro.tscn`, ~56 с): улица с машинами, кот сходит на тропу, табличка «халявная клубника через 500 метров» (текст обновлён автором 25.09.2026; прежний — «халявная клубника через 500 метров») крупным планом 3 с. Пропуск — Esc / Start / Enter (заглушка: автор кнопку не назначал). Как делать другие — [гайд по катсценам](../ГАЙД-КАТСЦЕНЫ.md).
- **Настройки** (`scenes/ui/settings_menu.tscn`, одна сцена для меню и паузы): режим окна, VSync, лимит кадров, разрешение 3D, сглаживание, тени, SSAO, свечение, туман, дальность, угол обзора, яркость, пиксельность, мышь, громкость.
- **Сохранения** (`scenes/save_game.gd`, `scenes/ui/save_menu.tscn`): 20 слотов с датой, переименование, удаление (в два нажатия), формат с номером версии. Пауза: «Сохранить / Загрузить». При выходе без сохранения — напоминание (текст — заглушка). Не сделано из канона: сложность, грибы, сон, контрольные точки, папки снимков.
- **Кот** (`scenes/player/cat_model.tscn`): воксельная модель, треугольные ушки, мордочка, красный браслетик; стоя неподвижен.
- Проверки: `tests/cat.gd`, `tests/start_area.gd`, `tests/menus_and_saves.gd`, `tests/intro.gd`, `tests/style_probe.gd`, `tests/first40.gd`.
- Пересобрать стартовое место и катсцену: `godot --path game/3d-probe -s res://tools/build_start_area.gd` (с окном).

## Проба графики «как в меню» (25.09.2026)

Кнопка «Проба графики» в главном меню открывает `scenes/style_probe/style_probe.tscn` — короткий кусок тропы (~7 м маршрута) в стиле картинки меню: мелкие разноцветные кубики, закатный свет, трава и цветы, забор у обрыва, долина с рекой и горы вдали. Участок 40 м ниже пока не переделан — ждёт ответа автора по пробе.

- модели (7 деревьев, куст, камень, бревно, забор, фонарь) — воксельные рецепты в `scenes/style_probe/voxel_recipes.gd`, готовые сцены в `scenes/style_probe/kinds/`;
- пересобрать: узел `Builder` → «Пересобрать пробу» → Ctrl+S, или `godot --path game/3d-probe -s res://tools/build_style_probe.gd` (с окном: трава и дальний лес — MultiMesh);
- проверки: `tests/style_probe.gd`.

## Новая сборка (25.09.2026): меню и первые 40 м, блочный пиксельный стиль

Игра начинается с **отдельной сцены меню** `scenes/menu/main_menu.tscn`: фон-картинка, версия 0.0.10, пиксельные шрифты (Tiny5, Press Start 2P — OFL), «Новая игра / Продолжить / Настройки / Выход». «Продолжить» неактивна, потому что сохранений ещё нет. В настройках есть «Пиксельность картинки» (выкл/×2/×3/×4). «Новая игра» открывает участок `scenes/world/first40.tscn`:

- мир блочный: блок 1×1×1, текстуры 16×16 (`assets/textures/pixel_own/`, рисует `tools/make_pixel_textures.py`), кот ≈ 2,5 блока и запрыгивает на 1 блок;
- тропа 5–11 блоков, петляет и сама сужается/расширяется; вид задаёт число `layout_seed` у узла `Builder`;
- 7 видов блочных деревьев (`scenes/trees/`: ель, ёлочка, сосна, берёза, дуб, большой дуб, сухостой), у каждого своя коллизия ствола;
- кусты проходимы, но замедляют вдвое; большие камни не пропускают, брёвна можно перепрыгнуть, мелкие камни не мешают;
- долина закрыта обрывами уступами по 4 блока, на 40 м — завал из брёвен; **невидимых стен нет**;
- квадратный рыжий кот собран из блоков прямо в `scenes/player/cat_player.tscn`: ходьба, бег (Shift / L2) с выносливостью, прыжок (Пробел / A), камера мышью или правым стиком, пауза Esc / Start.

Всё это — узлы сцены, их видно в редакторе. Землю и лес ставит сборщик: узел `Builder` → кнопка «Пересобрать участок» в инспекторе → Ctrl+S, или `tools/build_first40.gd` из консоли.

Всё, что ниже, описывает **старую пробу** (`main.tscn`, `scripts/`). Она больше не запускается из меню и оставлена архивом: механики оттуда переносятся в новую сцену по одной.
...
- Старт С1 (К2): после ввода имени идёт короткая катсцена — по улице в обе стороны едут машины, кот сходит на бетонно-каменную, затем лесную тропу, останавливается у таблички «халявная клубника через 500 метров» и получает управление в лесу на 0 м. Лес плотно ограничивает тропу с двух сторон: пройти можно по расчищенному пути. Дома в начале нет. Пустое имя даёт техническое имя по умолчанию; экранная клавиатура Deck пока не проверена на устройстве.
