...
- **Главное меню** (`scenes/menu/`): «Новая игра» → имя котика → стартовая катсцена → **стартовое место** (`scenes/world/start_area.tscn`): управление начинается там, где кончилась катсцена, первые 10 м тропы до временного завала. Можно вернуться к дороге — машина запускает концовку С9-машина («глупый суицидник»: полёт → отдаление → «Конец» → титры-заглушка → выход). Мир — общая сцена `start_area_world.tscn` для катсцены и игры. «Продолжить» — последний снимок, «Загрузить» — 20 слотов, «Настройки».
- **Катсцена С1** (`scenes/cutscene/intro.tscn`, ~56 с): улица с машинами, кот сходит на тропу, табличка «через 500 метров клубничные запасы» крупным планом 3 с. Пропуск — Esc / Start / Enter (заглушка: автор кнопку не назначал). Как делать другие — [гайд по катсценам](../ГАЙД-КАТСЦЕНЫ.md).
...
- Проверки: `tests/start_area.gd`, `tests/menus_and_saves.gd`, `tests/intro.gd`, `tests/style_probe.gd`, `tests/first40.gd`.
