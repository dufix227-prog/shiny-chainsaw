...
## Новая сборка (25.09.2026): меню и первые 40 м

Игра теперь начинается с **отдельной сцены меню** `scenes/menu/main_menu.tscn` (фон-картинка, версия 0.0.10, «Новая игра / Продолжить / Настройки / Выход»; «Продолжить» неактивна — сохранений ещё нет). «Новая игра» открывает участок `scenes/world/first40.tscn`:

- тропа 5–11 единиц шириной, петляет и сама сужается/расширяется; вид задаётся числом `layout_seed` у узла `Builder` (одно число — всегда тот же участок);
- лес из пяти видов деревьев (`scenes/trees/`: ель, сосна, берёза, дуб, сухостой) — у каждого дерева своя коллизия ствола, около 3100 деревьев;
- кусты проходимы, но замедляют вдвое; крупные камни и поваленные брёвна не пропускают; мелкие камни не мешают;
- долина закрыта крутыми склонами, на 40 м — завал из брёвен; **невидимых стен нет**;
- кот: ходьба, бег (Shift / L2) с выносливостью, прыжок (Пробел / A), камера мышью или правым стиком, пауза Esc / Start, счётчик «N м из 40».

Всё это — узлы сцены, видны в редакторе. Лес и землю ставит сборщик: узел `Builder` → кнопка «Пересобрать участок» в инспекторе → Ctrl+S, или `tools/build_first40.gd` из консоли. Деревья и кот пока временного вида: меш дерева меняется в одном месте — `scenes/trees/<вид>.tscn`.
